...
local function get_all_extensions()
	local exts = {}
	local ext_map = utils.get_supported_extensions()
		...
		default = default
			.new({
				output = function(selection)
					local _res = utils.from_path(selection.path, { chat_bufnr = SlashCommand.Chat.bufnr })
	...
	snacks = function(SlashCommand)
		...
		snacks = snacks.new({
			output = function(selection)
				...
				local _res = utils.from_path(path, { chat_bufnr = SlashCommand.Chat.bufnr })
				if type(_res) == "string" then
					return log:error(_res)
				end
				return SlashCommand:output(_res)
	...
	fzf_lua = function(SlashCommand)
		...
		fzf = fzf.new({
			...
			output = function(selected)
				...
				local _res = utils.from_path(file_path, { chat_bufnr = SlashCommand.Chat.bufnr })
				if type(_res) == "string" then
					return log:error(_res)
				end
				return SlashCommand:output(_res)
...
local choice = {
	...
	URL = function(SlashCommand, _)
		return vim.ui.input({ prompt = "Enter the URL: " }, function(url)
			...
			utils.from_url(url, { chat_bufnr = SlashCommand.Chat.bufnr }, function(_res)
				if type(_res) == "string" then
					return log:error(_res)
				end
...
function SlashCommand:output(selected, opts)
	...
	local encoded_attachment = utils.encode_attachment(selected)
